...
def comparative_averages(new_cases, states):
    for state in states:
        ...
        this_week_avg = sum(cases[-7:]) / 7
        prev_week_avg = sum(cases[:7]) / 7
        try:
            percent_change = ((this_week_avg - prev_week_avg) / prev_week_avg) * 100
        except ZeroDivisionError:
            percent_change = 0
        print(f"{state}:")
        print(f"This Week's Average: {this_week_avg:.2f}")
        print(f"Previous Week's Average: {prev_week_avg:.2f}")
